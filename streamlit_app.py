...
st.markdown(
    ...
    .stApp {background: linear-gradient(135deg,#0f1f18 0%, #122a21 50%, #0f1f18 100%);}    
    h1, h2, h3, h4 {font-family: 'Segoe UI', sans-serif;}
    .prob-table td {font-size: 0.9rem !important;}
    .prediction-badge {background:#1b5e20; color:#fff; padding:0.6rem 1rem; border-radius:999px; display:inline-block; font-weight:600;}
    .footer-note {font-size:0.75rem; opacity:0.7;}
    .dataset-badge {background:#263238; color:#d7ffd9; padding:0.35rem 0.65rem; border-radius:6px; font-size:0.7rem; margin-left:0.5rem;}
    </style>
    """,
...
st.caption("Trained on a curated Kaggle Plant Disease dataset (38 classes).")
st.write("Upload a clear leaf image to classify its disease and optionally generate structured treatment information.")
...
if uploaded:
    ...
    col_prev, col_meta = st.columns([3,2])
    with col_prev:
        st.image(pil_img, caption="Uploaded Image", use_container_width=True)
    with col_meta:
        st.markdown("### Image Details")
        st.write(f"Format: **{pil_img.format or 'N/A'}**")
        st.write(f"Size: **{pil_img.size[0]}×{pil_img.size[1]}**")
    ...
    st.markdown(f"<div class='prediction-badge'>Prediction: {pred_label}</div>", unsafe_allow_html=True)
    ...
    try:
        ...
        for rank, (idx, p) in enumerate(zip(topk.indices[0], topk.values[0]), start=1):
            rows.append({"Rank": rank, "Class": inv_map.get(int(idx), str(int(idx))), "Probability (%)": float(p)*100})
        st.subheader("Top Predictions")
        df = pd.DataFrame(rows)
        df_style = (df.style.hide(axis='index')
                        .format({"Probability (%)": "{:.2f}"})
                        .bar(color="#4caf50", subset=["Probability (%)"], vmin=0, vmax=100))
        st.write(df_style.to_html(classes="prob-table"), unsafe_allow_html=True)
    except Exception as e:
